...
# -f を持たない rm / mv をブロック（prezto safe-ops の -i エイリアス由来の「無言失敗」を止める）。
#
# この環境のエイリアス（~/.zprezto/modules/utility/init.zsh:71-76、実測で確認）:
#   rm='rm -i' / mv='mv -i' / cp='cp -i' / ln='ln -i'
# 2026-09-03 に scratchpad で実測した挙動:
#   -f 無しの rm  → `rm -i <path>` に展開。確認に答えられず削除されないまま exit 0（無言失敗）
#   -f 付きの rm  → `rm -i -f <path>`。-f が -i を上書きして削除される（man rm「-f overrides -i」）
#   -f 無しの mv  → `mv -i a b`。b が既存だと移動されないまま exit 0（rm と同じ無言失敗）
#   -f 付きの mv  → 上書きされる
#   cp / ln は -i 付きで衝突すると exit 1 で落ちるため無言失敗にならない → ここでは対象にしない
#     （cp は -f を足しても -i が勝つので、上書きしたいときは `command cp` を使う）
# したがって止めるべきは「-f を持たない rm / mv」だけ。
#
# 実測: bare-rm-blocked は全期間 271 件、うち 265 件（97%）が既に -f 付きで、
# ブロック→書き直しの往復コストだけ払っていた（~/.claude/guard-hits.log 全期間、2026-09-03 集計）。
# 残り 6 件はいずれも本当に -f 無しで、絞ったあとも従来どおり止まる。
# この 271 件は rm の実測で、mv の分は含まない。mv は「移動先が既存のときだけ無言失敗」
# なので、ただのリネームまで -f を要求することになる（＝新規に出る誤ブロック）。
# それでも対象に残すのは、フックの時点で移動先の存在を判定できず、危険なのは
# 「上書きのつもりが起きていない」側だから。回避は `mv -f` の 3 文字で済む。
# grep を 85% の無駄を根拠に絞ったのと同じ判断で、安全側の網（無言失敗の検知）は落としていない。
#
# grep/find と違い、判定は cmd_stripped ではなく生の cmd で行う（クォート内の言及でも発火する）。
# これは意図的な据え置き。cmd_stripped に替えて救えるのは実測 1.3% だけで、緩める根拠が無い。
#
# 正規の回避形である `command rm` / `sudo rm`（どちらもエイリアスを迂回する）は許可したいので、
# cmd_pos の command/sudo プレフィックス付きアンカーは使わず、区切り直後の rm / mv だけを見る。
# `command rm` / `git mv` は rm/mv が区切り直後に来ないため一致しない。
#
# 判定に perl を使うのは、ERE に否定先読みが無く「-f を含まない出現」を grep -E で書けないため
# （1 行に安全な形と危険な形が混在するので、出現ごとに見分ける必要がある）。
# /m を付けるのは -0777（全体を 1 スカラで読む）と組み合わせるため。付けないと ^ が
# 文字列先頭にしか当たらず、`cd /x\nrm foo` のように 2 行目以降にある rm を取りこぼす
# （grep は行単位だったので従来は拾えていた＝退行）。
# 引数側を [^|;&\n]* にするのも同じ理由で、改行をまたぐと次の行の -f（`grep -f pat` 等）を
# 自分の -f と誤読して、-f 無しの rm を素通りさせてしまう。
# -f の検出に --force も含める（`rm --force` は -[A-Za-z]*f に一致せず誤ブロックになる）。
unsafe_del="$(printf '%s' "${cmd}" | /usr/bin/perl -0777 -ne '
    while (/(?:^|[|;&(`\n]|\$\()[ \t]*(rm|mv)([ \t]+[^|;&\n]*|[ \t]*$)/gm) {
        my ($name, $args) = ($1, $2);
        next if $args =~ /(?:^|[ \t])(?:-[A-Za-z]*f|--force)/;
        print $name;
        last;
    }
' 2>/dev/null)"
if [ -n "${unsafe_del}" ]; then
    log_block "bare-${unsafe_del}-blocked" "${cmd}"
    if [ "${unsafe_del}" = "rm" ]; then
        echo "-f の無い素の rm は使わない。この環境の rm は -i エイリアスで、非対話実行だと削除されないまま exit 0 になる（無言失敗）。-f を付けるか command rm -f で実行し、削除後に ls で裏取りしてください。" >&2
    else
        echo "-f の無い素の mv は使わない。この環境の mv は -i エイリアスで、移動先が既存だと移動されないまま exit 0 になる（無言失敗）。-f を付けるか command mv で実行し、移動後に ls で裏取りしてください。" >&2
    fi
    exit 2
fi

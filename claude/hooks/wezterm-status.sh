...
# herdr のペイン内では表示を herdr に譲る。理由は2つあり、どちらも単独で根拠になる:
#  - herdr は同じ「どのエージェントが待っているか」をサイドバーに持つ（二重表示になる）。
#  - herdr の全ペインは、herdr（サーバー）を起動した 1 枚の WezTerm ペインの環境を継ぐため
#    WEZTERM_PANE が全員同じになる。join キーが潰れて全ペインが 1 つの状態ファイルを
#    奪い合い、タブは最後に書いた者の状態に化ける（実測: ペインが複数でも pane-0 のみ）。
# 書かないだけで、既存ファイルは消さない。継承した pane id は「同じ WezTerm ペインで

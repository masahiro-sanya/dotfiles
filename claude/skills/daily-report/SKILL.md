...
allowed-tools: Bash(git -C *), Bash(gh search prs *), Bash(gh pr list *), Bash(fd *), Bash(jq *), Bash(head *), Bash(cat *), Bash(date), Read, Write
---
...
デフォルトは **ローカル保存 + 提示まで**。ユーザーが投稿先（Slack チャンネル・Notion 等）を指定したときだけ、承認済み文面をそのまま投稿する。

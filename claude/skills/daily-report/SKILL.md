---
name: daily-report
description: 日報サイクル。朝は前日の採点（Notion 評価軸レビュー）と Notion「開発タスク一覧」の自分の担当タスクを踏まえて「今日のタスクと目標」を宣言、日中は宣言との途中経過を確認して軌道修正し、夜は当日の作業実績（コミット・PR 活動・Claude セッション・memory・Slack）をサブエージェントで横断収集して朝の宣言と突き合わせ、アウトカム中心の日報を作る。夜は加えて評価者目線の辛口採点を専用 Notion ログへ追記する。投稿は承認後のみ。Use when user says "日報", "daily report", "/daily-report", "今日のまとめ", "今日やること", "途中経過", "中間チェック", "日報 昼", "昼チェック", "評価チェック", "評価軸レビュー".
...
**このファイルを使う前（朝 step 2・朝 step 4 / 夜 step 5）に、まず `cat` で読む**。以降この文書の `{log_db}` `{criteria_page}` 等はここの値を指す。

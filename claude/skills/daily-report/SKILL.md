...
2. **基準ページが指す evidence の収集は investigator に委譲する**（この 3 ソースは独立なので 1 つの `investigator` にまとめて投げ、生データでなく「軸ごとに効く事実」の要約を返させる。cwd に関わらず絶対パス / ID で読ませる。daily-report は他プロジェクトから走ることが多く自動想起されないため取得先を明示的に渡す。取れない時は investigator に「取得失敗（理由）」と返させ、main は ⚠️ を明示して best-effort で採点し後続を止めない）。investigator に渡す取得先:

---
name: review-push
description: ローカルレビューを指摘ゼロまで収束させてから commit / push する個人ワークフロー。code-review --fix を反復し、通過した HEAD を記録して push する。レビューゲート付きリポ（.claude-review-gate）では push 前に必須。Use when user says "/review-push", "レビューしてpush", "レビュー通してプッシュ", "レビュー通ったらpush", "review push".
---

# review-push: ローカルレビュー収束 → commit → push

push する前にローカルでレビューを回し、指摘がゼロになった状態だけを外に出すための個人ワークフロー。`.claude-review-gate` があるリポでは PreToolUse hook（`guard-review-push.sh`）がこのスキルの通過記録なしの push をブロックする。

## 手順

### 1. 対象差分の確認

```bash
git status --short
git log --oneline @{upstream}..HEAD 2>/dev/null || git log --oneline origin/HEAD..HEAD
```

未コミットの変更 + 未 push のコミットがレビュー対象。差分ゼロなら「push するものがない」で終了。
feature branch であることを確認する（main 直コミット禁止）。

### 2. レビュー収束ループ（最大 3 回）

1. Skill ツールで `code-review` を `--fix` 付きで実行し、指摘の修正まで適用させる
   - **effort を必ず明示する。1 回目は `medium`、2 回目以降は `low`。** `code-review` は effort 無指定だと `codeReviewLastEffort`（前回打ったレベル）を再利用するので、一度でも `xhigh` / `max` を打つと以後の全実行がそこに貼り付く。この 2 つはレビュー用サブエージェントを撒く指示（finder の fan-out）が入るうえ、サブエージェント側はモデル指定を持たずセッションのモデル（Opus）を継承するため、収束ループ 3 回分がそのまま週次上限を削る。明示は「前回値の引き継ぎを断つ」のが主目的
   - **レベルは引数の先頭トークンで渡す**（`medium --fix` / `low --fix` のように、レビュー対象の指定より前に置く）。`--effort medium` のようなフラグ形式は解釈されず、レビュー対象の文字列に混ざるだけでレベルは前回のまま＝上の貼り付きが起きる
   - 1 回目を `medium` にするのは Opus 以外のモデルや将来の変更に備えた指定で、Opus 5 では `medium` と `high` が同じ内部セル（`o5-bmin`・finder 撒きなし）に落ちるため実際の差は出ない。**削減が効くのは 2 回目以降の `low`**（別セル）で、1 回目で洗った観点の残りを潰すだけなので低くて足りる
   - レビュー観点にはこのスキルと同じディレクトリの `checklist.md`（汎用レビューチェックリスト）を必ず含める。差分に該当する観点だけ適用し、プロジェクト固有の規約・チェックリストがあればそちらを優先して併用する。差分が再試行・時間の窓・上限・人が付ける印・外部 API 失敗に触れるときは、`~/.claude/rules/review-scenarios.md` の 5 シナリオをプロンプトに書いて渡す（code-review のサブエージェントは rules を読まない）
2. 修正が入ったら、**ビルド/テストの実行は `verify-runner` サブエージェントに委譲する**（`Task`）。verify-runner はプロジェクト標準のコマンド（Makefile / package.json / go.mod 等から判断）を回し、**通ったか／落ちたテスト名・`file:line`・原因ポインタだけ**を返す（ノイズの多い実行ログを main の文脈に持ち込まない）。テストの無効化・スキップは禁止。緑でなければ落ちた箇所を次のループの修正対象にする
3. 指摘ゼロになったらループを抜ける
4. 3 回で収束しない場合は**停止してユーザーに報告**する（発散。残った指摘の一覧と、どう振動しているかを添える）

### 3. commit

修正を意味のある単位で commit する。レビューと無関係な既存の未コミット変更（実行時設定など）は混ぜない。

### 4. 通過記録

```bash
git rev-parse HEAD >| "$(git rev-parse --absolute-git-dir)/claude-reviewed-sha"
```

**この記録は「いまの HEAD がレビュー通過済み」の証明。** 記録後に何かコミットしたら hook に止められるので手順 2 からやり直す。

### 5. push

```bash
git push
```

## リポでゲートを有効化するには

リポジトリの root に空ファイルを置くだけ（グローバル gitignore 済みなのでコミットには入らない）:

```bash
touch .claude-review-gate
```

## 注意事項

- **手順 5 で `bash-guard.sh` に止められることがある**（信頼 org 外への push は exit 2）。`~/.claude/hooks/gh-trusted-orgs.txt` に対象 owner が無いのが原因なので、レビューをやり直さず、ブロック文が出す 1 行（`echo '<owner>' >> ~/.claude/hooks/gh-trusted-orgs.txt`）をユーザーに提示して判断を仰ぐ。自分で追記しない・`bash-guard-ok` マーカーを勝手に作らない
- ゲートが検査するのは **Claude Code が実行する git push だけ**。ユーザーが自分のターミナルから push する分には関与しない
- 緊急バイパスは**ユーザーが明示的に承認した場合のみ** `CLAUDE_REVIEW_BYPASS=1 git push`。Claude の判断で勝手に使わない
- ゲートなしのリポでもこのスキル自体は実行できる（記録は残るが push は自由）
